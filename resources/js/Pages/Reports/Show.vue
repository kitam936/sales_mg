            ...
            <template #header>
                ...
                <div class="flex mt-4">
                    <div class="ml-2 md:ml-24 mt-0">
                        <Link as="button" :href="route('reports.index')" class="w-40 h-10 bg-indigo-500 text-sm text-white ml-0 hover:bg-indigo-600 rounded">Report一覧</Link>
                    </div>

                    <div class="ml-2 mb-0">
                        <Link as="button" :href="route('comments.create',{report:report.id})" class="w-40 h-10 bg-green-500 text-sm text-white ml-0 hover:bg-green-600 rounded">コメント登録</Link>
                    </div>
                </div>
            </template>
                            ...
                            <section class="text-gray-600 body-font relative">
                                ...
                                <div class="container px-2 py-2 mx-auto">
                                    ...
                                    <div class="flex flex-wrap -m-2">
                                        ...
                                        <div class="relative">
                                            <label for="id" class="py-1 text-sm text-gray-600">Report_ID</label>
                                            <div id="id" name="id" class="text-sm w-16 h-8 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 transition-colors duration-200 ease-in-out">{{ report.id }}</div>

                                        </div>
                                        <div class="flex">
                                        <div class="ml-0 md:ml-2 relative">
                                            <label for="co_id" class="py-1 text-sm text-gray-600">取引先</label>
                                            <div id="co_id" name="co_id" class="text-sm w-24 h-8 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 transition-colors duration-200 ease-in-out">{{ report.co_name }}</div>

                                        </div>
                                        <div class="ml-2 relative">
                                            <label for="shop_id" class="py-1 text-sm text-gray-600">店名</label>
                                            <div id="shop_id" name="shop_id" class="text-sm w-40 h-8 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 transition-colors duration-200 ease-in-out">{{ report.shop_name }}</div>
                                        ...
                                        <div class="relative">
                                            ...
                                            <div id="report" name="report" v-html="nl2br(report.report)" class="w-full bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 text-base outline-none text-gray-700 py-1 px-3 leading-6 transition-colors duration-200 ease-in-out resize-none overflow-auto">
                                            </div>
                                        </div>
                                        </div>
                                    ...
                                    <div v-if="report.user_id == login_user" class="ml-0 mt-4 mb-2 flex">

                                        <div class="p-2 w-full">
                                            <Link as="button" :href="route('reports.edit',{report:report.id})" class="w-40 h-10 flex mx-auto text-white bg-green-500 border-0 py-2 pl-10 focus:outline-none hover:bg-green-600 rounded text-sm">Report編集</Link>
                                        </div>
                                        <div v-if="!(comment_exist) " class="ml-0 p-2 w-full">
                                            <button class="w-40 h-10 flex mx-auto text-white bg-red-500 border-0 py-2 pl-12 focus:outline-none hover:bg-red-600 rounded text-sm" @click="deleteReport(report.id)" >削除する</button>
                                        </div>
                                    </div>
                                    </div>
                                    </div>
