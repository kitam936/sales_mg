<script setup>
    ...
    // ページネーション付きデータ
    const images = ref(props.images?.data ?? [])
    const links  = computed(() => props.images?.links ?? [])
    ...
    // フィルター変更時
    const submitFilters = () => {
        router.get(route('admin.hinban_image_check'), {
      ...
      const downloadCsv = () => {
        ...
        // 新しいタブで CSV をダウンロード
        window.open(`${route('admin.hinban_image_csv')}?${params}`, '_blank')
      }
            ...
            <table class="w-full text-center bg-white table-auto whitespace-no-wrap">
              <thead>
                <tr>
                  ...
                  <!-- <th class="w-2/15 py-1 text-sm bg-gray-100">季節</th> -->
                  <th class="w-1/15 py-1 text-sm bg-gray-100">UNIT</th>
                  <th class="w-1/15 py-1 text-sm bg-gray-100">Face</th>
                  <th class="w-2/8 py-1 text-sm bg-gray-100">品番</th>
                  <th class="w-5/8 py-1 text-sm bg-gray-100">品名</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="img in images" :key="img.hinban">
                  <td>{{ img.brand_id ?? '-' }}</td>
                  <!-- <td>{{ img.season_name ?? '-' }}</td> -->
                  <td>{{ img.unit_id ?? '-' }}</td>
                  <td>{{ img.face ?? '-' }}</td>
                  <td class="text-left">{{ img.hinban ?? '-' }}</td>
                  <td class="text-left">{{ img.hinban_name ?? '-' }}</td>
                </tr>
